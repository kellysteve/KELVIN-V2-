...
const { default: DareConnect, delay, PHONENUMBER_MCC, makeCacheableSignalKeyStore, useMultiFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto, Browsers} = require("@whiskeysockets/baileys")
...
async function startDare() {
const { state, saveCreds } = await useMultiFileAuthState("session")
const Dare = makeWASocket({
...
console.log(`This Code is Powered By Kelvin:`, code);
}
...
store.bind(Dare.ev)
    ...
    Dare.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            if (!Dare.public && !mek.key.fromMe && chatUpdate.type === 'notify') return
            if (mek.key.id.startsWith('BAE5') && mek.key.id.length === 16) return
            const m = smsg(Dare, mek, store)
            require("./kelvin")(Dare, mek, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    //autostatus view
        Dare.ev.on('messages.upsert', async chatUpdate => {
            ...
            if (mek.key && mek.key.remoteJid === 'status@broadcast') {
            	await Dare.readMessages([mek.key]) }
            }
    })
...
Dare.ev.on("connection.update",async  (s) => {
        const { connection, lastDisconnect } = s
        if (connection == "open") {
        	console.log(chalk.magenta(` `))
            console.log(chalk.yellow(`💫Connected to => ` + JSON.stringify(Dare.user, null, 2)))
			await delay(1999)
            console.log(chalk.yellow(`\n\n               ${chalk.bold.blue(`[ KELVIN-V2 ]`)}\n\n`))
    ...
    Dare.ev.on("messages.upsert",  () => { })

    Dare.sendText = (jid, text, quoted = '', options) => Dare.sendMessage(jid, {
        text: text,
        ...options
    }, {
        quoted,
    ...
    Dare.sendTextWithMentions = async (jid, text, quoted, options = {}) => Dare.sendMessage(jid, {
        text: text,
